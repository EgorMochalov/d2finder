                  ...
                  <Link key={p.id} to={`/profile/${p.id}`} className="glass rounded-xl p-3 md:p-4 glass-hover block stagger-enter" style={{ animationDelay: `${i * 0.05}s` }}>
                    <div className="flex items-start gap-2.5 md:gap-3">
                      <div className="relative shrink-0">
                        <div className="avatar w-10 h-10 md:w-11 md:h-11 text-base md:text-lg overflow-hidden">{p.avatarUrl ? <img src={p.avatarUrl} alt="" className="w-full h-full object-cover" /> : p.username[0].toUpperCase()}</div>
