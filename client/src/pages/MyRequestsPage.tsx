            ...
            {invitations.map((inv: any) => (
              <div key={inv.id} className="glass rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="avatar-square w-10 h-10 text-sm shrink-0">{inv.team?.logoUrl ? <img src={inv.team.logoUrl} alt="" className="w-full h-full object-cover" /> : (inv.team?.tag || '?')}</div>
          ...
          <div className="space-y-2">
            {requests.map((req: any) => (
              <div key={req.id} className="glass rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="avatar-square w-10 h-10 text-sm shrink-0">{req.team?.logoUrl ? <img src={req.team.logoUrl} alt="" className="w-full h-full object-cover" /> : (req.team?.tag || '?')}</div>
                  <div className="min-w-0">
