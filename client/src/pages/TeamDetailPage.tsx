        ...
        <div className="bg-gradient-to-r from-accent/10 via-blue/10 to-transparent p-6 md:p-8">
          <div className="flex items-center gap-4">
            <div className="relative shrink-0">
              <div className="avatar-square w-16 h-16 text-2xl">{team.logoUrl ? <img src={team.logoUrl} alt="" className="w-full h-full object-cover" /> : team.tag}</div>
              {isCaptain && <>
            ...
            {team.members?.map((m: any) => (
              <div key={m.id} className="flex items-center justify-between p-3 rounded-xl bg-surface border border-white/5 glass-hover">
                <div className="flex items-center gap-3">
                  <Link to={`/profile/${m.userId}`} className="avatar w-10 h-10 text-sm hover:ring-2 ring-accent/50 transition overflow-hidden">{m.user?.avatarUrl ? <img src={m.user.avatarUrl} alt="" className="w-full h-full object-cover" /> : (m.user?.username?.[0]?.toUpperCase() || '?')}</Link>
                  <div>
              ...
              <div className="space-y-2">
                {team.joinRequests.filter((r: any) => r.status === 'PENDING').map((req: any) => (
                  <div key={req.id} className="flex items-center justify-between p-3 rounded-xl bg-surface border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className="avatar w-10 h-10 text-sm overflow-hidden">{req.user?.avatarUrl ? <img src={req.user.avatarUrl} alt="" className="w-full h-full object-cover" /> : (req.user?.username?.[0]?.toUpperCase() || '?')}</div>
                      <div>
